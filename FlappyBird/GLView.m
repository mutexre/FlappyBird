...
- (BOOL)setupContext {
    ...
    if (!context || ![EAGLContext setCurrentContext:context]) return NO;
    if (context.API == kEAGLRenderingAPIOpenGLES3) isES3 = YES;
    return YES;
}
                  ...
                  colorFormat:(NSString*)colorFormat
{
    if (self = [super initWithFrame:frame]) {
        if (![self setupContext]) return nil;
        [self setupLayer:opaque retained:retained colorFormat:colorFormat];
        return self;
    }
    return nil;
}
...
- (instancetype)init {
    return [self initWithFrame:CGRectZero opaque:YES retained:NO colorFormat:kEAGLColorFormatRGBA8];
}
